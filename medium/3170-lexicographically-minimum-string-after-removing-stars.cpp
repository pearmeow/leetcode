 ...
 * characters. While there is a '*', do the following operation: Delete the leftmost '*' and the smallest non-'*'
...
class Solution {
   public:
    std::string clearStars(std::string s) {
        ...
        std::priority_queue<std::pair<char, size_t>, std::vector<std::pair<char, size_t>>, decltype(lexComp)>
            minChars(lexComp);
