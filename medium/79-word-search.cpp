...
bool helper(std::vector<std::vector<char>>& board, const std::string& word, int row, int col, int strInd,
    ...
    if (helper(board, word, row + 1, col, strInd + 1, used) || helper(board, word, row - 1, col, strInd + 1, used) ||
        helper(board, word, row, col + 1, strInd + 1, used) || helper(board, word, row, col - 1, strInd + 1, used)) {
        return true;
    }
    used[row][col] = 0;
    return false;
