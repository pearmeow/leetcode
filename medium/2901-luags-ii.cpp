 ...
 * for each j where 0 < j + 1 < k. words[ij] and words[ij+1] are equal in length, and the hamming distance between them
 * is 1, where 0 < j + 1 < k for all indices in the subsequence. Return a string array containing the words
 * corresponding to the indices (in order) in the selected subsequence. If there are multiple answers, return any of
 * them. Note: strings in words may be unequal in length.
 */
