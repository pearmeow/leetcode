...
double findEqValue(const std::vector<std::vector<std::string>>& equations,
        const std::vector<double>& values, const std::string& s1, const std::string& s2) {
...
double solveQuery(const std::vector<std::vector<std::string>>& equations,
    ...
    for (size_t i = 0; i < adjList.at(query[0]).size(); ++i) {
        initVal = findEqValue(equations, values, query[0], *adjList.at(query[0])[i]);
        theQueue.push(std::make_tuple(&query[0], adjList.at(query[0])[i], initVal));
    }
    ...
    while (!theQueue.empty()) {
        ...
        for (size_t i = 0; i < adjList.at(*nextStr).size(); ++i) {
            double multVal = std::get<2>(tup) * findEqValue(equations, values, *nextStr,
                                                            *adjList.at(*nextStr)[i]);
            theQueue.push(std::make_tuple(nextStr, adjList.at(*nextStr)[i], multVal));
        }
    }
...
std::vector<double> calcEquation(const std::vector<std::vector<std::string>>& equations,
        const std::vector<double>& values, const std::vector<std::vector<std::string>>& queries) {
