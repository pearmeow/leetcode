 ...
 * given an integer k which represents the maximum number of events you can attend. You can only attend one event
 * at a time. If you choose to attend an event, you must attend the entire event. Note that the end day is
        ...
        for (size_t offset = 1; offset <= events.size(); ++offset) {
            ...
            if (uniqueStarts[nextEvent - uniqueStarts.begin()] > endDay) {
                for (size_t j = 1; j < k; ++j) {
                    dp[lastDay][j] =
                        std::max(dp[lastDay][j], currEvent[2] + dp[nextEvent - uniqueStarts.begin()][j - 1]);
                }
            }
