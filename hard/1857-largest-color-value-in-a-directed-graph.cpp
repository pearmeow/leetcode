 ...
 * there is a directed edge from node aj to node bj. A valid path in the graph is a sequence of nodes x1 -> x2 ->
 * x3 -> ... -> xk such that there is a directed edge from x i to xi+1 for every 1 <= i < k. The color value of the
 * path is the number of nodes that are colored the most frequently occurring color along that path. Return the
 * largest color value of any valid path in the given graph, or -1 if the graph contains a cycle.
 */
