...
int modifiedKahnTopoSort(const std::string& colors, const std::vector<std::vector<int>>& adjList,
  ...
  for (size_t parent = 0; parent < parents.size(); ++parent) {
    for (size_t child = 0; child < adjList[parent].size(); ++child) {
      --indegrees[adjList[parent][child]];
    }
    indegrees[parents[parent]] = -1; // set to -1 to prevent readding to vector in future recursions
  }
  ...
  for (size_t parent = 0; parent < parents.size(); ++parent) {
    for (size_t child = 0; child < adjList[parent].size(); ++child) {
      for (size_t color = 0; color < COLORS; ++color) {
        pathValues[parent][color] = std::max(pathValues[parent][color], pathValues[child][color]);
      }
    }
    ++pathValues[parent][colors[parent]];
  }
  for (size_t parent = 0; parent < parents.size(); ++parent) {
    for (size_t color = 0; color < COLORS; ++color) {
      maxColor = std::max(pathValues[parent][color], maxColor);
    }
  }
  return maxColor;
}
